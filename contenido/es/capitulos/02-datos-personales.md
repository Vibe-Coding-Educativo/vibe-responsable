...
Un ejemplo en funcionamiento es [OpenWorksheets](https://openworksheets.github.io/), una aplicación libre de fichas interactivas que no tiene servidor ni cuentas: el alumnado descarga un archivo de entrega, que puede ir cifrado, y la aplicación avisa si ha sido manipulado. Otro ejemplo es [PCD](https://jjdeharo.github.io/pcd/), una plantilla correctora para exámenes tipo test: cada docente instala su propia copia sobre una hoja de cálculo, el alumnado responde sin registrarse y puede identificarse con un código en lugar de su nombre.
